...
if __name__ == '__main__':
    ...
    cmd_parser.add_argument('-mc', '--max-component', metavar='max_component', type=int, default=0, help='whether to keep only the maximal connected component, where voxels of distance no larger than `DISTANCE` are considered connected. Set to 0 to disable this function.')
    cmd_parser.add_argument('-sc', '--screenfile', metavar='screenfile', type=str, default=None, help='if you add the name in this parser, you can capture the png file of rendered view')
    ...
    visualization(voxels, threshold, title=str(ind+1)+'/'+str(voxels_raw.shape[0]), uniform_size=uniform_size, use_colormap=use_colormap, screenfile=screenfile)
