...
def generate_all_blocks(voxels, threshold=0.1, uniform_size=-1, use_colormap=False):
    ...
    Color is changed for each class. Block size is fixed.
    """
    ...
    for k in xrange(dims[2]):
        for j in xrange(dims[1]):
            for i in xrange(dims[0]):
                    ...
                    color = colorPalette[voxels[i][j][k].astype(np.int64)]
                    actors.append(block_generation([j+0.5, i+0.5, k+0.5, block_size, block_size, block_size], color=(color)))
                    counter = counter + 1
...
def display(actors, cam_pos, cam_vocal, cam_up, title=None, screenfile=None):
    ...
    if screenfile != None:
        ...
        writer.SetFileName(screenfile + '.png')
...
def visualization(voxels, threshold, title=None, uniform_size=-1, use_colormap=False, screenfile=None):
    """
    ...
    rad = math.pi * 0.4 #+ math.pi
    cam_pos = [center[0]* 10 * math.cos(rad), center[1] * 14 * math.sin(rad), center[2] * 3]

    display(actors, cam_pos, center, (1,0,0), title=title, screenfile=screenfile)
